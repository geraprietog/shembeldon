...
/**
 * Shembeldon Singles Championship – Neon UI (EN)
 * - Players
 * - Fixtures (weekly schedule) preloaded from seed.json
 * - Submit Results (Best-of-3: first to 2 sets)
 * - Standings (played, W/L, sets ±, games ±, win %)
 * - Data tab: import/export JSON (players, fixtures, matches)
...
export default function LeagueApp() {
  ...
  const [activeTab, setActiveTab] = useState("schedule"); // schedule | submit | standings | data

  useEffect(() => {
    saveData({ players, fixtures, matches });
  ...
  function parseSets(setsText) {
    // Accepts "6-4, 3-6, 10-7" (2 or 3 sets). Stops when someone reaches 2 sets.
    const parts = (setsText || "")
      .split(/[,\\n]/)
      .map((s) => s.trim())
      .filter(Boolean);
    const sets = [];
    ...
    for (const part of parts) {
      ...
      if (isNaN(ga) || isNaN(gb)) throw new Error(`Invalid set numbers: "${part}"`);
      sets.push({ ga, gb });
      if (ga > gb) wa++; else if (gb > ga) wb++;
      if (wa === 2 || wb === 2) break; // Best-of-3
    }
  ...
  function computeWinner(sets) {
    let wa = 0, wb = 0;
    for (const s of sets) {
      if (s.ga > s.gb) wa++; else if (s.gb > s.ga) wb++;
    }
    if (wa === wb) {
      // tiebreaker by total games if needed
      let gA = 0, gB = 0;
      for (const s of sets) { gA += s.ga; gB += s.gb; }
  ...
  function addMatch({ date, a, b, setsText, notes }) {
    ...
    let sets;
    try { sets = parseSets(setsText); } catch (e) { return alert(e.message); }
      ...
      <div className="max-w-6xl mx-auto">
        <header className="flex flex-col gap-2 mb-6">
          <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight">
            <span className="text-fuchsia-400 drop-shadow-[0_0_8px_rgba(244,114,182,0.6)]">Shembeldon</span>{" "}
            <span className="text-cyan-300 drop-shadow-[0_0_8px_rgba(103,232,249,0.6)]">Singles</span>{" "}
            <span className="text-lime-300 drop-shadow-[0_0_8px_rgba(190,242,100,0.6)]">Championship</span>
          </h1>
        ...
        {activeTab === "schedule" && (
          <ScheduleTab fixtures={fixtures} matches={matches} />
        )}
        {activeTab === "submit" && (
          <SubmitTab players={players} fixtures={fixtures} onAddPlayer={addPlayer} onRemovePlayer={removePlayer} onAddMatch={addMatch} matches={matches} onDeleteMatch={deleteMatch} />
        )}
        {activeTab === "standings" && (
          <StandingsTab standings={standings} />
        )}
        {activeTab === "data" && (
          <DataTab players={players} fixtures={fixtures} matches={matches} setAll={(p, f, m) => { setPlayers(p); setFixtures(f); setMatches(m); }} />
        )}

        <footer className="mt-10 text-xs text-neutral-500">
          <p>Neon prototype. Want a custom domain or publish to GitHub Pages? I can set it up.</p>
        </footer>
      </div>
    </div>
  );
  ...
  return (
    <button
      ...
      className={`px-3 py-1.5 rounded-2xl text-sm border transition ${
        active
          ? "bg-fuchsia-500/20 text-fuchsia-200 border-fuchsia-500 shadow-[0_0_12px_rgba(217,70,239,0.6)]"
...
function samePairDate(m, f) {
  ...
  // If fixture doesn't have date, match by week label if present
  if (!f.date && f.week && m.date) return true; // allow once
  return !f.date; // generic fixture without date
}
...
function SubmitTab({ players, fixtures, onAddPlayer, onRemovePlayer, onAddMatch, matches, onDeleteMatch }) {
  ...
  useEffect(() => {
    setForm((f) => ({ ...f, a: players[0] ?? "", b: players[1] ?? "" }));
  }, [players.length]);
  ...
  return (
    <div className="grid md:grid-cols-2 gap-6">
      <div className="bg-neutral-900 rounded-2xl border border-neutral-800 p-4">
        <h2 className="font-semibold text-lime-200 mb-3 drop-shadow-[0_0_6px_rgba(190,242,100,0.5)]">Players</h2>
        <div className="flex gap-2 mb-3">
          <input
            className="flex-1 border border-neutral-700 bg-black text-neutral-100 rounded-xl px-3 py-2"
      ...
      <div className="bg-neutral-900 rounded-2xl border border-neutral-800 p-4">
        <h2 className="font-semibold text-cyan-200 mb-3 drop-shadow-[0_0_6px_rgba(103,232,249,0.5)]">Submit result (Best-of-3)</h2>

        <label className="flex items-center gap-2 text-sm mb-3">
          <input type="checkbox" className="accent-fuchsia-500" checked={useFixture} onChange={(e) => setUseFixture(e.target.checked)} />
          Use scheduled match
        </label>
        ...
        {useFixture ? (
          <div className="grid grid-cols-2 gap-3 mb-3">
            <div className="col-span-2">
              ...
              <select className="w-full border border-neutral-700 bg-black rounded-xl px-3 py-2" value={fixtureId} onChange={(e) => {
                const id = e.target.value; setFixtureId(id);
                const fx = unplayed[parseInt(id,10)];
                if (fx) setForm((f) => ({ ...f, a: fx.a, b: fx.b, date: fx.date ?? new Date().toISOString().slice(0,10) }));
              }}>
                <option value="">—</option>
                {unplayed.map((f, idx) => (
                  <option key={idx} value={String(idx)}>
                    {(f.date || f.week || "") + " – " + f.a + " vs " + f.b}
                  </option>
                ))}
              </select>
            </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          <div className="col-span-2">
            ...
            <input type="date" className="w-full border border-neutral-700 bg-black rounded-xl px-3 py-2" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} />
          </div>
          <div>
            <label className="text-xs text-neutral-400">Player A</label>
            <select className="w-full border border-neutral-700 bg-black rounded-xl px-3 py-2" value={form.a} onChange={(e) => setForm({ ...form, a: e.target.value })}>
              {players.map((p) => <option key={p}>{p}</option>)}
            </select>
          </div>
          <div>
            <label className="text-xs text-neutral-400">Player B</label>
            <select className="w-full border border-neutral-700 bg-black rounded-xl px-3 py-2" value={form.b} onChange={(e) => setForm({ ...form, b: e.target.value })}>
              {players.map((p) => <option key={p}>{p}</option>)}
            </select>
          </div>
          <div className="col-span-2">
            <label className="text-xs text-neutral-400">Sets (e.g., 6-4, 3-6, 10-7)</label>
            <input className="w-full border border-neutral-700 bg-black rounded-xl px-3 py-2" value={form.setsText} onChange={(e) => setForm({ ...form, setsText: e.target.value })} />
          </div>
          <div className="col-span-2">
            <label className="text-xs text-neutral-400">Notes</label>
            <input className="w-full border border-neutral-700 bg-black rounded-xl px-3 py-2" placeholder="Optional" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} />
          </div>
        </div>
        <div className="flex gap-2">
          <button
            className="px-4 py-2 rounded-xl bg-fuchsia-600/30 text-fuchsia-200 border border-fuchsia-500"
            onClick={() => {
              onAddMatch(form);
              setForm((f) => ({ ...f, setsText: "6-4, 6-4", notes: "" }));
            }}
          >
            Save match
          </button>
      ...
      <div className="md:col-span-2 bg-neutral-900 rounded-2xl border border-neutral-800 p-4">
        <h2 className="font-semibold text-fuchsia-200 mb-3 drop-shadow-[0_0_6px_rgba(217,70,239,0.5)]">Recent matches</h2>
              ...
              {matches.map((m) => (
                <tr key={m.id} className="border-b border-neutral-800">
                  ...
                  <td className="py-2 pr-3 text-right">
                    <button className="text-xs text-red-400" onClick={() => onDeleteMatch(m.id)}>Delete</button>
                  </td>
                </tr>
              ))}
              {matches.length === 0 && (
                <tr><td colSpan={7} className="py-6 text-center text-neutral-500">No matches yet</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
function StandingsTab({ standings }) {
  return (
    <div className="bg-neutral-900 rounded-2xl border border-neutral-800 p-4">
      <h2 className="font-semibold text-lime-200 mb-3 drop-shadow-[0_0_6px_rgba(190,242,100,0.5)]">Standings</h2>
      <div className="overflow-auto">
        <table className="min-w-full text-sm">
          <thead>
            <tr className="text-left border-b border-neutral-800">
          ...
          <tbody>
            ...
            {standings.length === 0 && (
              <tr><td colSpan={8} className="py-6 text-center text-neutral-500">Add players to see the table</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
function ScheduleTab({ fixtures, matches }) {
  // Group by week label or by ISO week of date
  const byWeek = useMemo(() => groupByWeek(fixtures), [fixtures]);

  return (
    <div className="space-y-4">
      {Object.keys(byWeek).length === 0 && (
        <div className="bg-neutral-900 rounded-2xl border border-neutral-800 p-4 text-neutral-400">
      ...
      {Object.entries(byWeek).sort(([a],[b]) => a.localeCompare(b)).map(([wk, list]) => (
        <div key={wk} className="bg-neutral-900 rounded-2xl border border-neutral-800 p-4">
          <h3 className="font-semibold text-fuchsia-200 mb-3 drop-shadow-[0_0_6px_rgba(217,70,239,0.5)]">Week {wk}</h3>
          <div className="overflow-auto">
            <table className="min-w-full text-sm">
              <thead>
                <tr className="text-left border-b border-neutral-800">
                ...
                {list.map((f, idx) => {
                  ...
                  return (
                    <tr key={idx} className="border-b border-neutral-800">
                      ...
                      <td className="py-2 pr-3">{f.a} vs {f.b}</td>
                      <td className="py-2 pr-3">{played ? "Played" : "Pending"}</td>
                    </tr>
                  );
                })}
              </tbody>
...
function DataTab({ players, fixtures, matches, setAll }) {
  ...
  return (
    <div className="bg-neutral-900 rounded-2xl border border-neutral-800 p-4">
      <h2 className="font-semibold text-cyan-200 mb-3 drop-shadow-[0_0_6px_rgba(103,232,249,0.5)]">Export / Import</h2>
      ...
      <div className="mt-3 flex gap-2">
        ...
        <button className="px-4 py-2 rounded-xl bg-fuchsia-600/30 text-fuchsia-200 border border-fuchsia-500" onClick={() => {
          try {
            const obj = JSON.parse(raw);
            if (!Array.isArray(obj.players) || !Array.isArray(obj.fixtures) || !Array.isArray(obj.matches)) throw new Error("Invalid format");
            setAll(obj.players, obj.fixtures, obj.matches);
            alert("Data imported");
          } catch (e) {
...
function buildStandings(players, matches) {
  ...
  for (const m of matches) {
    ...
    for (const s of m.sets) {
      A.setsWon += s.ga > s.gb ? 1 : 0;
      A.setsLost += s.ga < s.gb ? 1 : 0;
      B.setsWon += s.gb > s.ga ? 1 : 0;
      B.setsLost += s.gb < s.ga ? 1 : 0;
      A.gamesWon += s.ga; A.gamesLost += s.gb;
      B.gamesWon += s.gb; B.gamesLost += s.ga;
    }
  }
  const rows = Array.from(map.values());
  rows.sort((r1, r2) => {
    const byWins = r2.wins - r1.wins;
    if (byWins) return byWins;
    const bySetDiff = (r2.setsWon - r2.setsLost) - (r1.setsWon - r1.setsLost);
    if (bySetDiff) return bySetDiff;
    const byGameDiff = (r2.gamesWon - r2.gamesLost) - (r1.gamesWon - r1.gamesLost);
    if (byGameDiff) return byGameDiff;
...
function groupByWeek(fixtures) {
  ...
  for (const f of fixtures) {
    const key = f.week ? String(f.week) : isoWeekKey(f.date);
    out[key] ||= [];
    out[key].push(f);
  }
